...
VoiceCloneBot 为大模型 Agent（如 OpenClaw）提供一套**完全解耦、高拟真度、轻量泛用的语音到语音（Voice-to-Voice）克隆插件**。
...
└────────────────────┬────────────────────────────┘
          ...
          └──────────┬──────────┘
                     │ HTTP POST /clone
          ┌──────────▼──────────┐
          │   server/app.py      │ ← FastAPI 常驻守护进程 (端口 8000)
